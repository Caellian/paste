...
mod test_to_lowercamel {
    use paste::paste;

    macro_rules! m {
        ($id:ident) => {
            paste! {
                const DEFAULT_CAMEL: &str = stringify!([<$id:lowerCamel>]);
                const LOWER_CAMEL: &str = stringify!([<$id:lowerCamel:lower>]);
                const UPPER_CAMEL: &str = stringify!([<$id:lowerCamel:upper>]);
            }
        };
    }
